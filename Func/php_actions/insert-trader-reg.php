...
if (isset($_POST['submit'])) {
    ...
    if (empty($username)) {
        ...
        if (($row = oci_fetch_array($username_query, OCI_BOTH)) != false){
            $error['userErr'] = "Username is taken";
        }
    }
    ...
    //Phone Number
    if (empty($phone)) {
        $error['phoneErr'] = "Phone number can not be empty";
    } 
    else if (!filter_var($phone, FILTER_VALIDATE_INT)) {
        $error['phoneErr'] = "Phone number should be in number";
    }

    //address
    if (empty($address)) {
        $error['addressErr'] = "Address can not be empty";
    } 
    ...
    if (count($error) == 0) {
        ...
        if(oci_execute($query)) {
            include "_verify_trader_email.php";
            $_SESSION['message'] = "Trader registered Successfully! Please wait for account verification";
